...
/* Private macro -------------------------------------------------------------*/
/* Socket Enumeration -----------------------------*/
enum { SOCK_ZERO, SOCK_ONE, SOCK_TWO, SOCK_THREE,
...
extern int BitCount, start, flag_uart, tmp_start;
...
// Each Axis data coming from Ethernet
struct EQ_DAQ_ONE DAQBoardOne;
...
void ProcessTextStream(uint8_t which, char *string) {
  ...
  switch(which) {
  case EQ_ONE : 
    result = strtok(string, "_"); // result contains Date. ex) 20130428
    printf("Date : %d", result);  // DEBUG
    DAQBoardOne.Date = result;
    result = strtok(string, "_"); // result contains Time. ex) 21384729
    DAQBoardOne.Time = result;
    result = strtok(string, "_"); // result contains Axis X Data. ex) +4096
    DAQBoardOne.AxisX = result;
    result = strtok(string, "_"); // result contains Axis Y Data. ex) +4096
    DAQBoardOne.AxisY = result;
    result = strtok(string, "_"); // result contains Axis Z Data. ex) +4096
    DAQBoardOne.AxisZ = result;
    break;
  case EQ_TWO : break;
  default: break;
  }
  ...
  while(1) {
    ...
    if(E1Flag) {
      ...
      for(order = 0; order < 100 ; order++) {
        ...
        // shape into meaningful formation        
    ...
    if(E2Flag) {
      ...
      for(order = 0; order < 100 ; order++) {
        ...
        // Only when socket is established, send data
        if(getSn_SR(SOCK_ONE) == SOCK_ESTABLISHED) {
          /* send the received data */
          send(SOCK_ONE, (uint8*)AxisData, strlen(AxisData), (bool)False);
        }
      }
    }
    
    /* Process server socket with each port */
    ProcessTcpClient(SOCK_ZERO);  // designated as for EQM-DAQ-01 with port 5050
    ProcessTcpClient(SOCK_ONE);   // designated as for EQM-DAQ-02 with port 6060
    ProcessTcpClient(SOCK_TWO);   // designated as for PC-CLIENT  with port 7070
    ProcessTcpClient(SOCK_THREE); // designated as for TOBEUSED   with port 8080
    
    /* Socket 4 to 7 reserved for future application
     * ProcessTcpClient(SOCK_FOUR);   // designated as for TOBEUSED with port 9090
     * ProcessTcpClient(SOCK_FIVE);   // designated as for TOBEUSED with port 10010
     * ProcessTcpClient(SOCK_SIX);    // designated as for TOBEUSED with port 10020
     * ProcessTcpClient(SOCK_SEVEN);  // designated as for TOBEUSED with port 10030
     */
  }
}
