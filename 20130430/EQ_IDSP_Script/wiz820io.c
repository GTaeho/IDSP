...
// Strng parse flag
uint8_t E1Flag = false;
uint8_t E2Flag = false;
uint8_t PCFlag = false;
  ...
  switch (getSn_SR(mSocket)) {  
    case SOCK_ESTABLISHED:
      ...
      if ((len = getSn_RX_RSR(mSocket)) > 0) {
        ...
        if(mSocket == SOCK_ZERO) {  // EQ-DAQ-01 board
          E1Flag = true;
        } else if(mSocket == SOCK_ONE) { // EQ-DAQ-02 board
          E2Flag = true; 
        } else if(mSocket == SOCK_TWO) { // PC Client
          PCFlag = true;
        }
