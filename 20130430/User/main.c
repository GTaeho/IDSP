...
void ProcessTextStream(uint8_t which, char *string) {
  ...
  case EQ_ONE : 
    ...
    printf("\r\nEQ_ONE, Date : %s", DAQBoardOne.Date);  // DEBUG
    result = strtok(NULL, "_"); // result contains Time. ex) 21384729
    DAQBoardOne.Time = result;
    printf("\r\nEQ_ONE, Time : %s", DAQBoardOne.Time);  // DEBUG
    result = strtok(NULL, "_"); // result contains Axis X Data. ex) +4096
    DAQBoardOne.AxisX = result;
    printf("\r\nEQ_ONE, AxisX : %s", DAQBoardOne.AxisX);  // DEBUG
    result = strtok(NULL, "_"); // result contains Axis Y Data. ex) +4096
    DAQBoardOne.AxisY = result;
    printf("\r\nEQ_ONE, AxisY : %s", DAQBoardOne.AxisY);  // DEBUG
    result = strtok(NULL, "_"); // result contains Axis Z Data. ex) +4096
    DAQBoardOne.AxisZ = result;
    printf("\r\nEQ_ONE, AxisZ : %s", DAQBoardOne.AxisZ);  // DEBUG
    break;
  case EQ_TWO :
    result = strtok(string, "_"); // result contains Date. ex) 20130428
    printf("Date : %d", result);  // DEBUG
    DAQBoardTwo.Date = result;
    break;
  default: break;
  }
}
...
int main(void) {
  ...
  while(1) {
    ...
    if(PCFlag) {
      ...
      char PC_Buf[45] = "Hello";
      // code for stacking algorithm which will combine data from two boards into one
      strncat(PC_Buf, DAQBoardOne.Date, 9);
      strncat(PC_Buf, DAQBoardOne.Time, 9);
