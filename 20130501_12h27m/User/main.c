...
void trim(uint8_t index) {
  if(!(mAxisBuf.tmp_data_x_lcd[index] & 0x2000)) { //when it is positive value
    if(mAxisBuf.tmp_data_x_lcd[index] & 0x1000) {
      mAxisBuf.tmp_data_x_lcd[index] = 0x0FFF;
    }
  } else { //when it is negative value
    mAxisBuf.tmp_data_x_lcd[index] = (~mAxisBuf.tmp_data_x_lcd[index] + 1) & 0x1fff;
    mAxisBuf.tmp_data_x_lcd[index] |= 0xE000;
  }
  ...
  if(!(mAxisBuf.tmp_data_y_lcd[index] & 0x2000)) {
    ...
    mAxisBuf.tmp_data_y_lcd[index] = (~mAxisBuf.tmp_data_y_lcd[index] + 1) & 0x1fff;
    mAxisBuf.tmp_data_y_lcd[index] |= 0xE000;
  }
  ...
  if(!(mAxisBuf.tmp_data_z_lcd[index] & 0x2000)) {
    ...
    mAxisBuf.tmp_data_z_lcd[index] = (~mAxisBuf.tmp_data_z_lcd[index] + 1) & 0x1fff;
    mAxisBuf.tmp_data_z_lcd[index] |= 0xE000;
  }  
}
