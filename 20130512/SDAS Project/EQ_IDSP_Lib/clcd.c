...
void CLCD_Configuration(void) {
  ...
  /* Pin Configuration -----------------------------------------------------------*/
  /* CLCD Pinmap
   * 1. VSS - GND       6. E - PA15         11. DB4 - PE11
   * 2. VDD - 3.3V      7. DB0 - PA8        12. DB5 - PE12
   * 3. V0 - GND        8. DB1 - PC7        13. DB6 - PE13
   * 4. RS - PB3        9. DB2 - PC6        14. DB7 - PE14
   * 5. R/W - PD3       10. DB3 - PD13
  */
  ...
  /* RW pin configuration */  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOD, &GPIO_InitStructure);
  
  /* E, DB0 pin configuration */  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15 | GPIO_Pin_8;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOA, &GPIO_InitStructure);
  
  /* DB1, DB2 pin configuration */  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_6;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOC, &GPIO_InitStructure);
  
  /* DB3 pin configuration */  
...
void CLCD_Handler(char rs, char data) {
  ...
  GPIO_WriteBit(GPIOA, GPIO_Pin_8, (data & 0x01) == Bit_SET ? Bit_SET : Bit_RESET);  // D0
  GPIO_WriteBit(GPIOC, GPIO_Pin_7, ((data & 0x02) >> 1) == Bit_SET ? Bit_SET : Bit_RESET);  // D1
  GPIO_WriteBit(GPIOC, GPIO_Pin_6, ((data & 0x04) >> 2) == Bit_SET ? Bit_SET : Bit_RESET);  // D2
  GPIO_WriteBit(GPIOD, GPIO_Pin_13, ((data & 0x08) >> 3) == Bit_SET ? Bit_SET : Bit_RESET);  // D3
