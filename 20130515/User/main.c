...
/* Timer 3 dedicate to server source */  
  ...
  while (1) {
    ...
    if(TenMilliSecCount >= 100) {  // 10 equals 10 millisecond
      TenMilliSecCount = 0;
    ...
    if(ParseUSART1) {
      ...
      printf("arrIdx : %d, buffer : %s", arrIdx, buffer);
      ...
      sprintf(buffer, "%+d,%+d,%+d\n",
              ...
              mAxisBuf.tmp_data_y_lcd[arrIdx],
              mAxisBuf.tmp_data_z_lcd[arrIdx]);
      printf("\r\narrIdx : %d, RX_BUF : %s", arrIdx, RX_BUF);
    ...
    if(RbitFlag) {
      ...
      // DEBUG
      mAxisX = mAxisBuf.tmp_data_x_lcd[arrIdx];
      mAxisY = mAxisBuf.tmp_data_y_lcd[arrIdx];
      mAxisZ = mAxisBuf.tmp_data_z_lcd[arrIdx];
      ...
      if(E1Flag) {
        ...
        if(getSn_SR(SOCK_ZERO) == SOCK_ESTABLISHED) {
          /* send selected data */
          send(SOCK_ZERO, (uint8_t*)E1_Buf, strlen(E1_Buf), (bool)false);
        }
      }
        
      if(E2Flag) {
        char E2_Buf[20];
        sprintf(E2_Buf, "%+d,%+d,%+d\n",
                mAxisBuf.tmp_data_x_lcd[arrIdx],
                mAxisBuf.tmp_data_y_lcd[arrIdx],
                mAxisBuf.tmp_data_z_lcd[arrIdx]);
        // Only when socket is established, allow send data
...
// following routine is only necessary when the board works as server
    ...
    if(E1Flag) {
      ...
      ProcessTextStream(EQ_ONE, RX_BUF, E1Order);
    ...
    if(E2Flag) {
      ...
      ProcessTextStream(EQ_TWO, RX_BUF, E2Order);
