...
int main(void) {
  ...
  while (1) {
    ...
    if(TIM5GLCDCount > 95) {  // Allocate proper time to share mcu resource with network
      ...
      x = mAxisNetBuf.axis_x_for_net[arrGLCDIdx];
      y = mAxisNetBuf.axis_y_for_net[arrGLCDIdx];
      z = mAxisNetBuf.axis_z_for_net[arrGLCDIdx];
      ...
      GLCD_DisplayKMAIntensity(KMAGrade);
    }
    ...
    if(ClientTimerCounter > 10) {  // 0 ~ 999 (1000) = 1 sec
      ...
      /* Prevent access to volatile variable warning */
      /* This have to be here in order to correct data to be used in ATFC */
      x = mAxisNetBuf.axis_x_for_net[arrIdx];
      y = mAxisNetBuf.axis_y_for_net[arrIdx];
      z = mAxisNetBuf.axis_z_for_net[arrIdx];
...
// following routine is only necessary when the board works as server
    ...
    if(E1Flag && E2FLag) {
      ...
      if(PCFlag) {
        // Send directly to PC
        SendToPC(EQ_BOTH, E1Order);
      }

      // Doesn't matter which variable to use. it only works as counter.       
      if(E1Order < 99) E1Order++;
      else E1Order = 0;
    }
