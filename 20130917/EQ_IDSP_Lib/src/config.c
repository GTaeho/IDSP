...
/* Data buffer big enough to contain 5 seconds of data */
static char EQ_DATA_BUF[20000] = {0};

// Each second count
int OneSecCount;

// Elapsed time in second scale
volatile int TimeElapsedInSecond = 0;
...
int MeanACC; 
volatile int MaxMeanACC;
int rMeantime;
int MaxMeanCount;
int ACCnArr[51];
int MaxMeanACCnArr[51];
...
void ATFC_GPIO_Configuration(void) {
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
  //GPIO_Init(GPIOD, &GPIO_InitStructure);  // This pin is no longer valid.
...
void CopyToTmpArray(uint8_t index) {
  ...
  if(SyncFlag) {
    mAxisBuf.tmp_data_x_lcd[mIndex] = mAxisData.data_x[mIndex];
  } else {
    while(SyncFlag == 0);
    mAxisBuf.tmp_data_x_lcd[mIndex] = mAxisData.data_x[mIndex];
  }
  ...
  if(SyncFlag) {
    mAxisBuf.tmp_data_y_lcd[mIndex] = mAxisData.data_y[mIndex];
  } else {
    while(SyncFlag == 0);
    mAxisBuf.tmp_data_y_lcd[mIndex] = mAxisData.data_y[mIndex];
  }
  ...
  if(SyncFlag) {
    mAxisBuf.tmp_data_z_lcd[mIndex] = mAxisData.data_z[mIndex] - 4095;
  } else {
    while(SyncFlag == 0);
    mAxisBuf.tmp_data_z_lcd[mIndex] = mAxisData.data_z[mIndex] - 4095;
  }
  ...
  if(SyncFlag) {
    mAxisBuf.ATFCBit_lcd[mIndex] = mAxisData.ATFCBit[mIndex];
  } else {
    while(SyncFlag == 0);
    mAxisBuf.ATFCBit_lcd[mIndex] = mAxisData.ATFCBit[mIndex];
  }
...
void CheckSignAndToInt(int index) {
  ...
  if(mAxisBuf.tmp_data_x_lcd[mIndex] & 0x2000) { //when it is negative value
    mAxisBuf.tmp_data_x_lcd[mIndex] = mAxisBuf.tmp_data_x_lcd[mIndex] & 0xFFF;
    mAxisBuf.tmp_data_x_lcd[mIndex] |= 0xFFFFF000;  // set MSB of int type
  }
  
  /* Axis Y --------------------------------------*/
  if(mAxisBuf.tmp_data_y_lcd[mIndex] & 0x2000) {
    mAxisBuf.tmp_data_y_lcd[mIndex] = mAxisBuf.tmp_data_y_lcd[mIndex] & 0xFFF;
    mAxisBuf.tmp_data_y_lcd[mIndex] |= 0xFFFFF000;  // set MSB of int type
  }
  
  /* Axis Z --------------------------------------*/
  if(mAxisBuf.tmp_data_z_lcd[mIndex] & 0x2000) {
    mAxisBuf.tmp_data_z_lcd[mIndex] = mAxisBuf.tmp_data_z_lcd[mIndex] & 0xFFF;
    mAxisBuf.tmp_data_z_lcd[mIndex] |= 0xFFFFF000;  // set MSB of int type
  }
}

int ExtractMaxMeanACC(int data) {
  int mMeanACCn, result;
  mMeanACCn = data;
  
  if(MaxMeanCount < ACCnFifty - 1) {
...
void CopyToFatFsDataBuffer(int index) {  
  ...
  while(!SyncFlag); // Synchronize to get rid of unpleasant odd data.
  ...
  strcat(EQ_DATA_BUF, strBuf);
  
  // Count each second to write to sd card.
  if(arrIdx == 99) {
    OneSecCount++;
    TimeElapsedInSecond++;
    ...
    if(OneSecCount == 5) {
      OneSecCount = 0;
      //printf("\r\nstrlen(EQ_DATA_BUF) : %d", strlen(EQ_DATA_BUF));
      ...
      int bytesWritten;
      bytesWritten = f_printf(&fsrc, EQ_DATA_BUF);
      //f_printf(&fsrc, EQ_DATA_BUF);
      printf("\r\n%d of bytesWritten", bytesWritten);
      
      /* Flush */
      if(TimeElapsedInSecond != 180) {
        f_sync(&fsrc);
      }
      
      // Reset DATA1_BUF
      // switch data buffer on each second to alleviate the load.
      memset(EQ_DATA_BUF, 0, sizeof(EQ_DATA_BUF));
      //EQ_DATA_BUF[0] = 0;
      
      //printf("\r\nTimeElapsedInSecond = %d", TimeElapsedInSecond);
      
      if(TimeElapsedInSecond == 180) {  // 179 (180sec) equals 3 minute
...
void MultipleBoardDataToSendToPC(int which, int orderOne, int orderTwo) {
  ...
  case EQ_BOTH:
    ...
    if(GoATFCFlag) {
      // Apply ATFC Algorithm to Averaged Axis Z data
      ATFCAlgorithm(AvgZ);
